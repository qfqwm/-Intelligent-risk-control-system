<template>
  <div class="all">
    <!-- 左边资产目录区域 -->
    <ScriptClassification></ScriptClassification>
    <!-- 右边数据展示区域 -->
    <div class="right">
      <!-- 搜索区域 -->
      <a-form
      ...
      >
        <a-form-item has-feedback label="脚本状态：" name="checkPass" style="width: 41%">
          <a-input v-model:value="formState.checkPass" type="text" autocomplete="off" />
        </a-form-item>
        <a-form-item name="area" label="脚本名称" :rules="[{ message: 'Missing area' }]" style="width: 41%">
          <a-select :options="areas" />
        </a-form-item>
        <a-form-item name="area" label="脚本类型" :rules="[{ message: 'Missing area' }]" style="width: 41%">
          <a-select :options="areas" />
        </a-form-item>
      ...
      <div style="display: flex; justify-content: end">
        ...
        <a-button type="primary" style="margin-left: 15px"> 新增脚本 </a-button>
      </div>
      <!-- 表格区域 -->
      <a-table
...
<script lang="ts" setup>
  import ScriptClassification from '@/pages/Scriptment/component/ScriptClassification.vue';
